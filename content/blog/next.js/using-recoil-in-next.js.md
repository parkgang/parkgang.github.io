...
> 아마 테스트 해보시면 비동기 콜을 하는 page를 다른 페이지에서 link를 통해 들어가면 문제가 없지만 해당 page 주소로 바로 접속하면 아래와 같은 에러가 출력됩니다.
> ![](./images/using-recoil-in-next.js-7.png)
> axios 에서 return 데이터 중 `.data` 만을 사용하지 않고 모두 던지면 호출 기록은 있지만 렌더링하다가 next.js에서 run time error 가 발생합니다.  
> 해당 문제는 `npm run dev` 일 때만 발생합니다. 같은 코드인데 build 되어 start으로 실행 된 next.js server에서는 해당 문제가 발생하지 않습니다.
...
우리는 next.js의 api를 사용하고 있기 떄문에 build 시점에 next.js server가 내려가 있으니 api에 호출에 실패에서 발생하는 메시지 입니다.
...
결론적으로 경고 메시지가 출력되어도, catch에 걸리더라도 구어진 정적파일은 문제가 없으며 next.js server가 실행했을 때 정작적으로 동작합니다.
...
에러가 생각 디테일 하지 않았고 CSR에서는 되던건게 안되서 특히 axios 결과 값을 모두 return 했을 때 next.js server가 down되는 부분에서 많은 시간을 소비하였습니다.
