...
이번엔 Webhooks을 생성해서 원하는 채널에 메시지를 게시할 수 있도록 해보겠습니다.

추후, 해당 Webhooks으로 CI/CD 성공 메시지를 보내도록 디자인할 것 입니다.

> 참고로 수신 웹후크 (Incoming Webhooks App) 의 경우 Deprecated 된거 같습니다.

## 채널 생성

Webhooks으로 메시지를 게시할 채널을 생성하도록 합니다. 저는 추후에 CI/CD로 사용할 채널을 미리 만들어 사용할 것입니다.

![](./images/workflows-with-aks-github-slack-5/17-0.png)

## App 생성

[Slack API App](https://api.slack.com/apps) 으로 접속하여 App을 생성하도록 합니다.
...
App의 아이콘과 이름을 수정하도록 합니다. 저는 CI/CD Bot으로 사용할 것이기 때문에 아래와 같이 꾸며보았어요. 😛

![](./images/workflows-with-aks-github-slack-5/18-1.png)

## Webhooks 생성

위에서 생성한 App에서 아래의 빨간 네모박스를 선택합니다.

![](./images/workflows-with-aks-github-slack-5/19.png)

토글버튼을 이용하여 활성화 후 Workspace를 선택합니다.
...
아래의 URL으로 요청 시 지정된 채널에 메시지를 전송할 수 있습니다.

![](./images/workflows-with-aks-github-slack-5/22.png)

![](./images/workflows-with-aks-github-slack-5/23.png)

테스트 결과 정상적으로 전송되는 것을 확인할 수 있습니다.

![](./images/workflows-with-aks-github-slack-5/24.png)

# 마무리

이번 시간을 통하여 부족한 것이 많지만 `ChatOps` 의 첫 걸음으로도 볼 수 있는 작업들을 진행하였습니다.

앞으로 github에서 무슨 일이 발생하는지 빠르게 모니터링하고 대응할 수 있을 것입니다.  
이외에도 slack에서 프로젝트와 관련된 많은 이야기가 오고 갈 것입니다.

이제 관련된 설정은 모두 마쳤고 마지막 단계인 자동화 부분이 남았네요! 거의 다 왔습니다.
